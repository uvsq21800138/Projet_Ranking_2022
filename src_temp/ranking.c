...
double epsi = 1e-6;
double alpha = 0.85;
long int vertices_count = 0;
long int edges_count = 0;
...
void axP(double *npi, double *opi, DATA *T)	{
	init_tab_double(vertices_count,npi, 0.0);
...
void PageRank(FILE *F, FILE *O) {
	...
	vertices_count = read_file_param(F);
	edges_count = read_file_param(F);
	
	DATA *P = init_tab_list(vertices_count);
	int *f = calloc(vertices_count,sizeof(int));
	double **pi = calloc(2,sizeof(double*));
	pi[0] = calloc(vertices_count,sizeof(double));
	pi[1] = calloc(vertices_count,sizeof(double));
	init_tab_double(vertices_count,pi[0],(double) 1/vertices_count);
	
	// lecture des valeurs de P dans T et f
	read_file_list(F,vertices_count,P,f);
	
	int cmpt = 0;
	double xf = 0.0;
	double scal = 0.0;
	double inv = (double) 1/vertices_count;

	// premier tour
	xf = mult_vect_lc(vertices_count,pi[0], f);
	axP(pi[1], pi[0], P);
	scal = (double)((1-alpha)*inv) + (double)(alpha * inv * xf);
	add_vect_scal(vertices_count, pi[1], scal);
	cmpt++;
	// boucle 
	while(norme1_dif(pi[0], pi[1]) > epsi)	{
		xf = mult_vect_lc(vertices_count,pi[cmpt%2], f);
		axP(pi[1-cmpt%2], pi[cmpt%2], P);
		scal = (double)((1-alpha)*inv) + (double)(alpha * inv * xf);
		add_vect_scal(vertices_count, pi[1-cmpt%2], scal);
		cmpt++;
	...
	ecriture_res(O,vertices_count,pi[1-cmpt%2],cmpt, alpha, epsi);
	
	free_tab_list(vertices_count,P);
	free(f);
	free(pi[0]);
	free(pi[1]);
	free(pi);
